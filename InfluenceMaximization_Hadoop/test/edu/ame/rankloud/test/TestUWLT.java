...
public class TestUWLT {
	...
	public static void main(String[] args) throws Exception {
		...
		String usage = new String("Usage: TestUWLT [cascade | simulation | optimization]");
		
		if (1 != args.length) {
			System.out.println(usage);
			System.exit(1);
		}
		else if (0 == args[0].compareToIgnoreCase("cascade")) {
			runCascade(args);
		}
		
		else if (0 == args[0].compareToIgnoreCase("simulation")) {
			runSimulation(args);
		}
		
		else if (0 == args[0].compareToIgnoreCase("optimization")) {
			runOpt(args);
		}
